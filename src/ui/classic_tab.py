"""
Classic Tab UI for KJScreener Streamlit app.
...
def _render_results(configManager):
    ...
    bc.download_button(
        ...
        file_name=f'KJScreener_results_{datetime.datetime.now().strftime("%H:%M:%S_%d-%m-%Y")}.csv',
        mime='text/csv',
        type='secondary',
        width='stretch',
    )
        ...
        with st.spinner(f"Loading {selected_stock} data..."):
            try:
                ...
                if data is not None and not data.empty:
                    ...
                    st.plotly_chart(ohlc_fig, width='stretch')
                else:
