"""
AI-Native Tab UI for KJScreener Streamlit app.
...
def _session_db_path() -> str:
    base = os.path.join(os.path.dirname(_src_dir), '.KJScreener_sessions')
...
def _get_or_create_agent_session():
    ...
    if '_agent_sql_session' not in st.session_state:
        try:
            ...
            # Prefer cached real openai-agents; fall back to sys.path surgery
            _real = sys.modules.get('_KJScreener_openai_agents_real')
            if _real is not None and hasattr(_real, 'SQLiteSession'):
                SQLiteSession = _real.SQLiteSession
            else:
...
def _check_credential_source() -> str:
    ...
    env_key = _os.environ.get('KJScreener_API_KEY', '')
    if api_key and api_key != env_key:
        key_source = "browser localStorage / manual input"
    elif api_key:
        key_source = "env var KJScreener_API_KEY"
    else:
        key_source = "not set"
...
def render():
    ...
    # ── Hydrate LLM credentials (only seed keys that widgets have not created) ─
    # AI Native runs before the Configuration tab widgets. Seed missing keys from
    # localStorage / YAML / env. Never overwrite keys that already exist — widgets
    # own them once instantiated (StreamlitAPIException otherwise).
    try:
        import classes.BrowserConfigStore as BrowserConfigStore
        _llm = BrowserConfigStore.load_llm_config() or {}
        if 'ai_provider' not in st.session_state and _llm.get('provider'):
            st.session_state['ai_provider'] = _llm['provider']
        if 'ai_model' not in st.session_state and _llm.get('model'):
            st.session_state['ai_model'] = _llm['model']
        if 'ai_base_url' not in st.session_state and _llm.get('base_url') is not None:
            st.session_state['ai_base_url'] = _llm.get('base_url') or ''
        if not st.session_state.get('ai_api_key'):
            remembered = ''
            if _llm.get('remember_api_key') and _llm.get('api_key'):
                remembered = _llm.get('api_key') or ''
            if remembered:
                st.session_state['ai_api_key'] = remembered
            else:
                import os as _os
                env_key = (
                    _os.environ.get('KJScreener_API_KEY', '')
                    or _os.environ.get(_llm.get('api_key_env') or 'KJScreener_API_KEY', '')
    ...
    # Ensure local agents package wins over openai-agents on sys.modules
    if _src_dir not in sys.path:
        sys.path.insert(0, _src_dir)
    try:
        import agents as _local_agents
        # Check the package's actual __file__ path rather than hasattr'ing
        # submodules like agent_loader/llm_config — those are only set as
        # attributes AFTER something has explicitly imported them, so on a
        # fresh process (before any persona/tool code has run) this check
        # would false-positive as "not local" even when 'agents' already
        # correctly resolved to our own package, forcing an unnecessary
        # (and, historically, destructive) self-heal reimport on every very
        # first render(). Comparing __file__ against our known local
        # src/agents/ directory is deterministic regardless of import order.
        _local_agents_file = getattr(_local_agents, '__file__', '') or ''
        _is_local = os.path.dirname(os.path.abspath(_local_agents_file)) == os.path.abspath(
            os.path.join(_src_dir, 'agents')
        )
        if not _is_local:
            # Drop only the top-level 'agents' name so a fresh `import agents`
            # resolves to our local package. Deliberately do NOT pop real
            # openai-agents submodules (agents.models, agents.models.interface,
            # agents.mcp, agents.agent, ...) — our local package has no
            # submodules with those names, so leaving them cached is harmless,
            # and it's required: the real Agent class does
            # `from .models.interface import Model` lazily, only when Agent(...)
            # is actually instantiated later. Wiping those cache entries here
            # makes that later import fail (or resolve to a second, distinct
            # copy of the same class), which is exactly what caused the
            # recurring "Agent model must be a string, Model, or None, got
            # OpenAIChatCompletionsModel" warning / Groq 404 bug.
            for k in [k for k in list(sys.modules) if k == 'agents']:
                if not k.startswith('_KJScreener'):
    ...
    # Read credentials from session_state (hydrated above / set in Configuration tab)
    provider = st.session_state.get('ai_provider', 'openai')
    model = st.session_state.get('ai_model', 'openai/gpt-oss-120b')
    api_key = st.session_state.get('ai_api_key', '')
    ...
    with col_side:
        ...
        if persona_names:
            ...
            if selected_persona:
                ...
                _all_tools = [
                    ...
                    'get_index_quote', 'screen_52_week_high',
                ]
                ...
                # This selector previously had no effect — the agent always
                # used the persona's saved YAML tool list regardless of what
                # was picked here. Apply the in-session selection for real
                # (session-only; doesn't touch the persona's YAML file).
                selected_persona = dict(selected_persona)
                selected_persona['tools'] = _selected_tools
        else:
        ...
        if not api_key:
            st.warning("No API key — go to **Configuration** tab, paste key, tick Remember, Save.", icon="⚠️")
        else:
            st.caption(f"LLM: `{provider}` / `{model}`" + (f" @ {base_url}" if base_url else ""))
        ...
        def _md_to_html(text: str) -> str:
            ...
            for line in lines:
                ...
                if stripped.startswith("|") and stripped.endswith("|"):
                    ...
                    if not in_table:
                        in_table = True
                        th = "".join(f"<th style='border:1px solid #444;padding:5px 10px;background:#1a1a2e;color:#f63366;text-align:left;white-space:normal;word-break:break-word;max-width:220px;'>{c}</th>" for c in cells)
                        table_rows.append(f"<tr>{th}</tr>")
                    else:
                        td = "".join(f"<td style='border:1px solid #333;padding:5px 10px;white-space:normal;word-break:break-word;max-width:220px;'>{c}</td>" for c in cells)
                        table_rows.append(f"<tr>{td}</tr>")
                else:
                    if in_table:
                        result_lines.append(f"<div style='overflow-x:auto;max-width:100%;margin:6px 0;'><table style='border-collapse:collapse;font-size:0.85rem;'>{''.join(table_rows)}</table></div>")
                        table_rows = []
                        in_table = False
        ...
        for msg in history:
            if msg['role'] == 'user':
                ...
                chat_inner += (
                    ...
                    "<div class='chat-label'>KJScreener</div>"
      ...
      requestAnimationFrame(function() {{
        ...
        if submitted and user_input.strip():
            ...
            try:
                ...
                # Prefer session values; fall back to yaml defaults so we never
                # send an empty/wrong model (SDK would then use gpt-5.* default).
                _prov = (provider or '').strip() or 'openai-compatible'
                _mod = (model or '').strip()
                _base = (base_url or '').strip() or None
                if not _mod or _mod.startswith('gpt-5'):
                    try:
                        import classes.BrowserConfigStore as _BCS
                        _stored = _BCS.load_llm_config() or {}
                        if not _mod:
                            _mod = (_stored.get('model') or '').strip()
                        if not _base:
                            _base = (_stored.get('base_url') or '').strip() or None
                        if _prov in ('', 'openai') and _stored.get('provider'):
                            _prov = _stored.get('provider') or _prov
                    except Exception:
                        pass
                if not _mod:
                    _mod = 'openai/gpt-oss-120b'
                if _prov == 'openai-compatible' and not _base:
                ...
                if (kite_sess and kite_sess.is_connected
                    ...
                    try:
                        # Pass the ScreeniAgent wrapper (not the raw Agent) so
                        # openai-compatible providers can use the direct
                        # tool-loop and avoid the Runner/MultiProvider 404 bug.
                        result = kite_sess.run_agent_query(
                            agent_obj, query, sql_session=sql_session,
                        )
                    except Exception:
...
def _render_kite_auth_compact():
    ...
    if not kite_cfg.get('enabled'):
        # Don't vanish silently — show why, so a Docker/server deployment
        # where Kite unexpectedly disappears is diagnosable from the UI
        # itself instead of needing `docker exec` + guesswork.
        with st.expander("🔑 Kite (disabled)", expanded=False):
            st.caption(kite_cfg.get('_debug', 'kite_mcp.enabled is false in KJScreener.yaml.'))
            st.caption(
                "Fix: set `kite_mcp.enabled: true` in KJScreener.yaml and make sure that file is "
                "actually copied into the image, OR set env var `KJSCREENER_KITE_ENABLED=true` on "
    ...
    if authenticated and session_alive:
        ...
        if st.button("Disconnect", key='kite_disconnect_btn', width='stretch'):
    ...
    if pending_url:
        ...
        c1, c2 = st.columns(2)
        if c1.button("✅ Done", key='kite_confirm_login', width='stretch'):
            st.session_state['_kite_authenticated'] = True
            st.session_state.pop('_kite_pending_login_url', None)
            st.rerun()
        if c2.button("🔄", key='kite_refresh_link', width='stretch'):
            st.session_state.pop('_kite_pending_login_url', None)
            st.session_state.pop('_kite_mcp_session', None)
            st.rerun()
        return
    ...
    if st.button("🔗 Connect", key='kite_connect_btn', width='stretch'):
