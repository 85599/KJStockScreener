...
# Pyinstaller compile Windows: pyinstaller --onefile --icon=src\icon.ico src\KJScreener.py  --hidden-import cmath --hidden-import talib.stream --hidden-import numpy --hidden-import pandas --hidden-import alive-progress --hidden-import chromadb
# Pyinstaller compile Linux  : pyinstaller --onefile --icon=src/icon.ico src/KJScreener.py  --hidden-import cmath --hidden-import talib.stream --hidden-import numpy --hidden-import pandas --hidden-import alive-progress --hidden-import chromadb
...
def main(testing=False, testBuild=False, downloadOnly=False, execute_inputs:list = [], isDevVersion=None, backtestDate=date.today()):
    ...
    if tickerOption == 'W' or tickerOption == 'N' or tickerOption == 'E' or (tickerOption >= 0 and tickerOption < 17):
        ...
        if totalConsumers == 1:
            ...
            # Screening is mostly I/O-bound (waiting on NSE/yfinance network
            # calls per stock), not CPU-bound, so running noticeably more
            # worker processes than CPU cores lets the machine keep fetching
            # the next stock's data while earlier ones are still waiting on
            # the network — this is the main lever for full-universe (2500+
            # stock) scan speed. Capped at 16 to avoid hammering NSE/yfinance
            # hard enough to trigger rate-limiting, which would silently drop
            # results instead of just being slower.
            totalConsumers = min(totalConsumers * 2, 16)
