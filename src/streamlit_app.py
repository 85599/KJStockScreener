...
tab_screen, tab_ai, tab_gap, tab_optchain, tab_yf, tab_config, tab_psc, tab_opt, tab_blog, tab_about = st.tabs([
    '📊 Classic Screen',
    '🤖 AI Native',
    '🔮 Gap Prediction',
    '⛓️ Option Chain',
...
with tab_yf:
    ...
    st.caption('Enter the code for a stock (e.g. RELIANCE, TCS, INFY) — `.NS` is added automatically for NSE. For US stocks, use the ticker directly (AAPL, NVDA).')
    ...
    try:
        ...
        _yf_available = False
        st.error('The `yfinance` library is not installed. Run `pip install yfinance` in your terminal.', icon='📦')

    if _yf_available:
        def _yf_normalize_ticker(raw: str) -> str:
            t = raw.strip().upper()
            if not t:
                return t
            if '.' in t or t.startswith('^'):
                return t
            if t in ('NIFTY', 'NIFTY50', '^NSEI'):
                return '^NSEI'
            if t in ('BANKNIFTY', '^NSEBANK'):
                return '^NSEBANK'
        ...
        def _fmt_num(val, prefix='', suffix='', decimals=2, indian=False):
            if val is None or (isinstance(val, float) and math.isnan(val)):
                return '—'
            try:
                v = float(val)
                # Auto-detect Indian scale when ₹ is used
                use_indian = indian or (prefix == '₹')
                if use_indian:
                    if abs(v) >= 1e12:  # >= 1 Lakh Cr
                        return f'{prefix}{v/1e12:.{decimals}f} Lakh Cr{suffix}'
                    if abs(v) >= 1e7:   # >= 1 Cr
                        return f'{prefix}{v/1e7:,.{decimals}f} Cr{suffix}'
                    if abs(v) >= 1e5:   # >= 1 Lakh
                        return f'{prefix}{v/1e5:.{decimals}f} Lakh{suffix}'
        ...
        def _fmt_range(lo, hi, prefix='₹'):
            if lo is None and hi is None:
                return '—'
            return f'{_fmt_price(lo, prefix)} – {_fmt_price(hi, prefix)}'

        col_search, col_period, col_btn = st.columns([3, 1.2, 1])
        yf_ticker_raw = col_search.text_input(
            'Stock / Index Ticker',
            value=st.session_state.get('yf_last_ticker', 'RELIANCE'),
            placeholder='RELIANCE, TCS, AAPL, NVDA, NIFTY…',
            key='yf_ticker_input',
        )
        yf_period = col_period.selectbox(
            'History Period',
            options=['1mo', '3mo', '6mo', '1y', '2y', '5y', 'max'],
            index=3,
            key='yf_period',
        )
        fetch_btn = col_btn.button('🔍 Fetch Data', type='primary', width='stretch', key='yf_fetch')

        if fetch_btn or st.session_state.get('yf_auto_load'):
            if not yf_ticker_raw.strip():
                st.warning('Please enter a ticker.', icon='⚠️')
            else:
                ticker_sym = _yf_normalize_ticker(yf_ticker_raw)
                st.session_state['yf_last_ticker'] = yf_ticker_raw.strip().upper()
                st.session_state['yf_auto_load'] = True
                ...
                if not info and hist.empty:
                    st.error(f'No data found for `{ticker_sym}`. Check the ticker (NSE stocks need the `.NS` suffix).', icon='❌')
                else:
                    ...
                    _is_inr = (currency == 'INR')
                    _price_str = _fmt_price(price, cur_sym) if price else '—'
                    _delta_str = f'{change:+.2f} ({change_pct:+.2f}%)' if change is not None else ''
                    _delta_color = '#26a69a' if (change is not None and change >= 0) else '#ef5350'
                    _day_r = _fmt_range(info.get('dayLow'), info.get('dayHigh'), cur_sym)
                    _w52_r = _fmt_range(info.get('fiftyTwoWeekLow'), info.get('fiftyTwoWeekHigh'), cur_sym)
                    _vol = _fmt_num(info.get('volume') or info.get('regularMarketVolume'), decimals=1, indian=_is_inr)
                    _mcap = _fmt_num(info.get('marketCap'), prefix=cur_sym, decimals=2)
                    _avgv = _fmt_num(info.get('averageVolume'), decimals=1, indian=_is_inr)

                    st.markdown(f"""
                    <style>
                      .lq-grid {{ display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); gap:0.6rem; margin:0.6rem 0 1rem 0; }}
                      .lq-card {{ background:#1a1d24; border:1px solid #30363d; border-radius:10px; padding:0.7rem 0.85rem; }}
                      .lq-label {{ font-size:0.72rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:0.25rem; }}
                      .lq-value {{ font-size:1.05rem; font-weight:700; color:#f0f6fc; word-break:break-word; line-height:1.25; }}
                      .lq-delta {{ font-size:0.8rem; font-weight:600; margin-top:0.2rem; }}
                      @media (max-width:900px) {{ .lq-grid {{ grid-template-columns:repeat(3,1fr); }} }}
                    </style>
                    <div class="lq-grid">
                      <div class="lq-card"><div class="lq-label">Price</div><div class="lq-value">{_price_str}</div>
                        <div class="lq-delta" style="color:{_delta_color}">{_delta_str}</div></div>
                      <div class="lq-card"><div class="lq-label">Day Range</div><div class="lq-value">{_day_r}</div></div>
                      <div class="lq-card"><div class="lq-label">52W Range</div><div class="lq-value">{_w52_r}</div></div>
                      <div class="lq-card"><div class="lq-label">Volume</div><div class="lq-value">{_vol}</div></div>
                      <div class="lq-card"><div class="lq-label">Market Cap</div><div class="lq-value">{_mcap}</div></div>
                      <div class="lq-card"><div class="lq-label">Avg Volume</div><div class="lq-value">{_avgv}</div></div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with yf_sub[0]:
                        if not hist.empty:
                            st.line_chart(hist[['Close']], height=380, width='stretch')
                            with st.expander('OHLCV Table (last 30 rows)'):
                                st.dataframe(hist.tail(30).style.format('{:.2f}'), width='stretch', height=320)
                        else:
                            st.info('No historical data found.')

                        st.markdown('<p class="section-header">Key Statistics</p>', unsafe_allow_html=True)
                        k1, k2, k3, k4 = st.columns(4)
                        k1.metric('Trailing P/E', _fmt_num(info.get('trailingPE')))
                        k2.metric('Forward P/E', _fmt_num(info.get('forwardPE')))
                        k3.metric('PEG Ratio', _fmt_num(info.get('pegRatio')))
                        k4.metric('Price/Book', _fmt_num(info.get('priceToBook')))
                        k5, k6, k7, k8 = st.columns(4)
                        k5.metric('EPS (TTM)', _fmt_num(info.get('trailingEps'), prefix=cur_sym))
                        dy = info.get('dividendYield')
                        k6.metric('Dividend Yield', f'{dy*100:.2f}%' if dy else '—')
                        k7.metric('Beta', _fmt_num(info.get('beta')))
                        k8.metric('50 / 200 DMA', f"{_fmt_num(info.get('fiftyDayAverage'))} / {_fmt_num(info.get('twoHundredDayAverage'))}")

                        # Quick technical read — computed from the history we
                        # already fetched, no extra API calls needed.
                        if not hist.empty and len(hist) >= 15:
                            st.markdown('<p class="section-header">Technical Summary</p>', unsafe_allow_html=True)
                            closes = hist['Close']
                            delta = closes.diff()
                            gain = delta.clip(lower=0).rolling(14).mean()
                            loss = (-delta.clip(upper=0)).rolling(14).mean()
                            rs = gain / loss.replace(0, float('nan'))
                            rsi14 = float((100 - (100 / (1 + rs))).iloc[-1]) if not rs.isna().all() else None
                            ...
                            if rsi14 is not None:
                                rsi_read = 'Overbought' if rsi14 >= 70 else ('Oversold' if rsi14 <= 30 else 'Neutral')
                                t1.metric('RSI (14)', f'{rsi14:.1f}', rsi_read)
                            if sma20 is not None:
                                trend20 = 'Above 20-SMA (bullish bias)' if last_close > sma20 else 'Below 20-SMA (bearish bias)'
                                t2.metric('vs 20-day SMA', _fmt_price(sma20, cur_sym), trend20)
                            if sma50 is not None:
                                trend50 = 'Above 50-SMA' if last_close > sma50 else 'Below 50-SMA'
                                t3.metric('vs 50-day SMA', _fmt_price(sma50, cur_sym), trend50)
                            st.caption('Technical summary is calculated directly from the price history above (simple RSI-14 and SMA) — for informational purposes only, not trading advice.')
                    ...
                    with yf_sub[1]:
                        ...
                        with c_right:
                            ...
                            profile_rows = {
                                ...
                                'Employees': _fmt_num(info.get('fullTimeEmployees'), decimals=0),
                                'Website': info.get('website'),
                            }
                            officers = info.get('companyOfficers') or []
                            if officers:
                                profile_rows['Key Officer'] = officers[0].get('name')
                    ...
                    # 3. Financials
                    with yf_sub[2]:
                        fin_type = st.radio('Statement', ['Income Statement', 'Balance Sheet', 'Cash Flow'], horizontal=True, key='yf_fin_type')
                        period_type = st.radio('Period', ['Annual', 'Quarterly'], horizontal=True, key='yf_fin_period')
                        try:
                            if fin_type == 'Income Statement':
                                df_fin = tk.income_stmt if period_type == 'Annual' else tk.quarterly_income_stmt
                            elif fin_type == 'Balance Sheet':
                                df_fin = tk.balance_sheet if period_type == 'Annual' else tk.quarterly_balance_sheet
                            else:
                                df_fin = tk.cashflow if period_type == 'Annual' else tk.quarterly_cashflow
                            if df_fin is not None and not df_fin.empty:
                                st.dataframe(df_fin.style.format('{:,.0f}'), width='stretch', height=420)
                            else:
                                st.info('No financial statement data available for this ticker.')
                        except Exception as e:
                    ...
                    with yf_sub[3]:
                        ...
                        a1.metric('Target Mean', _fmt_num(info.get('targetMeanPrice'), prefix=cur_sym))
                        a2.metric('Target High', _fmt_num(info.get('targetHighPrice'), prefix=cur_sym))
                        a3.metric('Target Low', _fmt_num(info.get('targetLowPrice'), prefix=cur_sym))
                        a4.metric('Recommendation', str(info.get('recommendationKey', '—')).title())

                        try:
                            rec = tk.recommendations
                            if rec is not None and not rec.empty:
                                st.markdown('**Recent Recommendations**')
                                st.dataframe(rec.tail(15), width='stretch', height=280)
                        except Exception:
                            pass
                        try:
                            rec_sum = tk.recommendations_summary
                            if rec_sum is not None and not rec_sum.empty:
                                st.markdown('**Recommendations Summary**')
                                st.dataframe(rec_sum, width='stretch')
                        except Exception:
                            pass
                    ...
                    with yf_sub[4]:
                        ...
                        try:
                            ...
                            for item in news_list:
                                ...
                                if title.strip().lower() not in seen_titles:
                                    ...
                                    combined_news.append({'title': title, 'link': link, 'publisher': pub or 'Yahoo Finance', 'pubDate': pub_time})
                        except Exception:
                            pass

                        # Second source — Google News RSS covers Indian financial
                        # press (Moneycontrol, ET, LiveMint, etc.) that Yahoo's
                        # own feed frequently has nothing for on NSE-listed names.
                        try:
                            gnews_query = f'{name} stock' if name and name != ticker_sym else ticker_sym.replace('.NS', '')
                            for item in _fetch_google_news_rss(gnews_query):
                        ...
                        if not combined_news:
                            st.info('No recent news found for this ticker from any source.')
                        else:
                            for item in combined_news[:20]:
                                title, pub, link, pub_time = item['title'], item['publisher'], item['link'], item['pubDate']
                                if link:
                    ...
                    with yf_sub[5]:
                        if hist.empty or len(hist) < 2:
                            st.info('Not enough price history to compute pivot levels.')
                        else:
                            prev_day = hist.iloc[-1]
                            p_high, p_low, p_close = float(prev_day['High']), float(prev_day['Low']), float(prev_day['Close'])
                            ...
                            # Fibonacci pivots
                            fib_range = p_high - p_low
                            fib_r1 = pivot + 0.382 * fib_range
                            fib_s1 = pivot - 0.382 * fib_range
                            fib_r2 = pivot + 0.618 * fib_range
                            fib_s2 = pivot - 0.618 * fib_range
                            fib_r3 = pivot + 1.000 * fib_range
                            fib_s3 = pivot - 1.000 * fib_range

                            st.caption(f'Based on the last completed session — High {_fmt_price(p_high, cur_sym)}, Low {_fmt_price(p_low, cur_sym)}, Close {_fmt_price(p_close, cur_sym)}.')
                            ...
                            with pv1:
                                ...
                                st.dataframe(pd.DataFrame({
                                    ...
                                    'Price': [r3, r2, r1, pivot, s1, s2, s3],
                                }).style.format({'Price': lambda v: _fmt_price(v, cur_sym)}), width='stretch', hide_index=True, height=280)
                            with pv2:
                                st.markdown('**Fibonacci Pivots**')
                                st.dataframe(pd.DataFrame({
                                    'Level': ['R3', 'R2', 'R1', 'Pivot', 'S1', 'S2', 'S3'],
                                    'Price': [fib_r3, fib_r2, fib_r1, pivot, fib_s1, fib_s2, fib_s3],
                                }).style.format({'Price': lambda v: _fmt_price(v, cur_sym)}), width='stretch', hide_index=True, height=280)

                            st.caption('Pivot points are a standard technical-analysis tool — computed directly from the last session\'s High/Low/Close, not a prediction. For informational purposes only, not trading advice.')
                        ...
                        with d1:
                            ...
                            try:
                                ...
                                if divs is not None and not divs.empty:
                                    st.dataframe(divs.tail(20).to_frame('Dividend'), width='stretch', height=300)
                                else:
                                    st.caption('No dividend history.')
                            except Exception:
                        ...
                        with d2:
                            ...
                            try:
                                ...
                                if splits is not None and not splits.empty:
                                    st.dataframe(splits.to_frame('Split Ratio'), width='stretch', height=300)
                                else:
                                    st.caption('No split history.')
                            except Exception:
                                st.caption('Split data unavailable.')

                        st.markdown('<p class="section-header">Actions (combined)</p>', unsafe_allow_html=True)
                        try:
                            actions = tk.actions
                            if actions is not None and not actions.empty:
                                st.dataframe(actions.tail(25), width='stretch', height=280)
                        except Exception:
                            pass
                    ...
                    with yf_sub[7]:
                        ...
                        with h1:
                            ...
                            try:
                                ...
                                if ih is not None and not ih.empty:
                                    st.dataframe(ih, width='stretch', height=280)
                                else:
                                    st.caption('—')
                            except Exception:
                                st.caption('—')
                        with h2:
                            ...
                            try:
                                mfh = tk.mutualfund_holders
                                if mfh is not None and not mfh.empty:
                                    st.dataframe(mfh, width='stretch', height=280)
                                else:
                                    st.caption('—')
                            except Exception:
                                st.caption('—')
                            ...
                            try:
                                ...
                                if it is not None and not it.empty:
                                    st.dataframe(it.head(15), width='stretch', height=280)
                                else:
                                    st.caption('—')
                            except Exception:
                                st.caption('—')
