...
art = colorText.GREEN + r'''
  _  KJ   _____                                 
 | |/ /  / ____|                                
 | ' /  | (___   ___ _ __ ___  ___ _ __   ___ _ __ 
 |  <    \___ \ / __| '__/ _ \/ _ \ '_ \ / _ \ '__|
 | . \   ____) | (__| | |  __/  __/ | | |  __/ |   
 |_|\_\ |_____/ \___|_|  \___|\___|_| |_|\___|_|   
...
class tools:
    ...
    def showDevInfo():
        ...
        print(colorText.BOLD + colorText.WARN +
              "\n[+] Developer: Khushal Jain." + colorText.END)
        print(colorText.BOLD + colorText.WARN +
              ("[+] Version: %s" % VERSION) + colorText.END)
        print(colorText.BOLD +
              "[+] Home Page: https://github.com/85599/KJStockScreener" + colorText.END)
        print(colorText.BOLD + colorText.FAIL +
              "[+] Read/Post Issues here: https://github.com/85599/KJStockScreener/issues" + colorText.END)
        print(colorText.BOLD + colorText.GREEN +
              "[+] Join Community Discussions: https://github.com/85599/KJStockScreener/discussions" + colorText.END)
        print(colorText.BOLD + colorText.BLUE +
              "[+] Download latest software from https://github.com/85599/KJStockScreener/releases/latest" + colorText.END)
        input('')
    ...
    def isClosingHour():
        curr = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
        openTime = curr.replace(hour=18, minute=30)
        closeTime = curr.replace(hour=19, minute=00)
        return ((openTime <= curr <= closeTime) and (0 <= curr.weekday() <= 4))

    def getNiftyModel(proxyServer=None):
        """
        Load the bundled Nifty-50 gap-direction model weights as a plain numpy
        .npz file (extracted from the original Screeni-py trained Keras model
        — same weights, same StandardScaler mean/scale — just re-implemented
        as a manual forward pass). This deliberately avoids a tensorflow /
        scikit-learn dependency: those are heavy (500MB+) and tensorflow in
        particular has spotty pip wheel availability on some Python/Windows
        combos, causing "No matching distribution found for tensorflow"
        install failures. Returns a lightweight callable `model(x)` plus a
        dict with 'scaler' (has .transform()) and 'columns', mirroring the
        original model/pkl interface so callers don't need to change.
        """
        import numpy as np
        ml_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'ml')
        npz_path = os.path.join(ml_dir, 'nifty_model_weights.npz')
        if not os.path.isfile(npz_path):
            raise FileNotFoundError(
                f"Bundled model weights missing at {npz_path}. "
                f"Re-download/extract the KJScreener zip — src/ml/nifty_model_weights.npz "
                f"should be included."
            )

        npz = np.load(npz_path, allow_pickle=True)
        n_layers = sum(1 for k in npz.files if k.startswith('w'))
        ...
        def model(x):
            """Manual forward pass — numerically identical to the original
            trained Keras dense network (verified against it directly)."""
            x = np.asarray(x, dtype=np.float64)
            for i in range(n_layers):
                w = npz[f'w{i}']
                b = npz[f'b{i}']
                act = str(npz['activations'][i])
                x = x @ w + b
                x = relu(x) if act == 'relu' else sigmoid(x)
            return x

        model.predict = lambda x, verbose=0: model(x)  # keras-style .predict() shim
        ...
        pkl = {
            'scaler': _Scaler(npz['scaler_mean'], npz['scaler_scale']),
    ...
    def loadStockData(stockDict, configManager, proxyServer=None):
        ...
        if os.path.exists(cache_file):
            ...
            cache_url = "https://raw.github.com/85599/KJStockScreener/actions-data-download/actions-data-download/" + cache_file
            ...
            if resp.status_code == 200:
                ...
                print(colorText.BOLD + colorText.GREEN +
                      "[+] Downloading cache from KJScreener server for faster processing, Please Wait.." + colorText.END)
                try:
                ...
                print(colorText.BOLD + colorText.FAIL +
                      "[+] Cache unavailable on KJScreener server, Continuing.." + colorText.END)
...
def isDocker():
    if 'KJScreener_DOCKER' in os.environ:
        return True
    return False

def isGui():
    if 'KJScreener_GUI' in os.environ:
        return True
    return False
