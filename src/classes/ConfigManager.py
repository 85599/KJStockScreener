...
try:
    from classes.Utility import isGui, isDocker
except Exception:
    def isGui():
        return 'KJScreener_GUI' in os.environ
    def isDocker():
        return os.environ.get('KJScreener_DOCKER', '').upper() in ('TRUE', '1', 'YES')
...
class tools:
        ...
        if default:
            ...
            try:
                ...
                if showFileCreatedText:
                    ...
                    print(colorText.BOLD + colorText.GREEN +
                        '[+] Use Configuration tab in GUI to edit in future.' + colorText.END)
                    # GUI / Docker: never block on input() or exit — Streamlit must keep running
                    if isGui() or isDocker():
                        print(colorText.BOLD + colorText.GREEN +
                            '[+] Continuing in GUI mode with default config.' + colorText.END)
                    else:
                        print(colorText.BOLD + colorText.GREEN +
            ...
            print('')
            print(colorText.BOLD + colorText.GREEN +
                  '[+] KJScreener User Configuration:' + colorText.END)
            self.period = input(
    ...
    def getConfig(self, parser):
        if len(parser.read('KJScreener.ini')):
            try:
                ...
                if not (isGui() or isDocker()):
                    input(colorText.BOLD + colorText.FAIL +
                          '[+] KJScreener requires user configuration again. Press enter to continue..' + colorText.END)
                parser.remove_section('config')
                # In GUI/Docker always regenerate defaults non-interactively
