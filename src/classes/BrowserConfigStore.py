...
- Fallback: KJScreener.ini / KJScreener.yaml on disk (still used by CLI)
...
def _get_storage():
    ...
    try:
        ...
        if hasattr(st, "session_state"):
            if _KEY_STORAGE not in st.session_state:
                st.session_state[_KEY_STORAGE] = {}
            if "_bcs_storage_init" not in st.session_state:
                st.session_state["_bcs_storage_init"] = True
...
def _find_KJScreener_yaml() -> str:
    """Locate KJScreener.yaml relative to this file or repo root."""
    candidates = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'KJScreener.yaml'),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'KJScreener.yaml'),
        'KJScreener.yaml',
    ]
    for p in candidates:
...
def save_screening_config(data: dict, fallback_cm) -> None:
    """Persist screening params to localStorage and mirror to KJScreener.ini.

    Args:
        data: dict with screening param values (keys match ConfigManager attrs).
        fallback_cm: ConfigManager.tools() instance used to write KJScreener.ini.
    """
    storage = _get_storage()
...
def load_llm_config(fallback_yaml_path: str = None) -> dict:
    """Load LLM config from localStorage; fall back to KJScreener.yaml.

    Args:
        fallback_yaml_path: Optional explicit path to KJScreener.yaml.
    ...
    if storage is not None:
        ...
        if data:
            return data
        # streamlit-local-storage sometimes exposes values via storedItems
        # before getItem stabilises across reruns
        try:
            items = getattr(storage, 'storedItems', None) or {}
            if isinstance(items, dict) and _KEY_LLM in items:
                data = _safe_json_load(items.get(_KEY_LLM))
                if data:
                    return data
        except Exception:
            pass
    ...
    yaml_path = fallback_yaml_path or _find_KJScreener_yaml()
    defaults = {
        "provider": "openai-compatible",
        "model": "openai/gpt-oss-120b",
        "base_url": "https://api.groq.com/openai/v1",
        "api_key": os.environ.get("KJScreener_API_KEY", "") or os.environ.get("GROQ_API_KEY", ""),
...
def save_llm_config(data: dict, remember_api_key: bool, fallback_yaml_path: str = None) -> None:
    """Persist LLM config to localStorage and mirror safe fields to KJScreener.yaml.
