...
from classes.KJScreenerTA import ScreenerTA
try:
    import chromadb
...
class tools:
    ...
    def getNiftyPrediction(self, data, proxyServer=None):
        """
        Real ML gap-direction prediction for NIFTY 50 — same trained weights
        as the upstream Screeni-py project's AI model (a small dense NN
        trained on Nifty/Gold/Crude daily % change), re-implemented as a
        plain numpy forward pass (see Utility.getNiftyModel) instead of
        loading a tensorflow/keras model, so no tensorflow install is needed.
        `data` must be the frame from Fetcher.fetchLatestNiftyDaily() —
        i.e. NIFTY OHLC concatenated with gold_Close / crude_Close columns.
        Returns (pred, trend, confidence, data_used_df).
        """
        import warnings
        ...
        with SuppressOutput(suppress_stderr=True, suppress_stdout=True):
            ...
            for col in pkl['columns']:
                d[col] = d[col].pct_change(fill_method=None) * 100
            d = d.ffill().dropna()
            d = d.iloc[-1]
            d_scaled = pkl['scaler'].transform([d])
            pred = model.predict(d_scaled, verbose=0)[0]

        trend = 'BEARISH' if pred > 0.5 else 'BULLISH'
        confidence = Utility.tools.getSigmoidConfidence(pred[0])
        if isGui():
            return pred, trend, confidence, pd.DataFrame(datacopy.iloc[-1]).T
        return pred

    def getIndexGapHeuristic(self, index_name, data):
        """
        Statistical (NOT machine-learning) gap-direction heuristic for indices
        that don't have a trained model bundled (Bank Nifty, Sensex — the
        real NN above is trained on NIFTY 50 data only). Uses the index's own
        recent daily OHLC: last-day momentum, distance from its 5-day mean,
        and how often it has historically gapped in the same direction as the
        prior day's close move, blended into a 0-100 confidence score via a
        logistic squashing function (same shape as getSigmoidConfidence, but
        derived from real technical features instead of a trained network).
        `data` must be at least ~10 daily rows with Open/High/Low/Close.
        Returns (trend, confidence, data_used_df) — CLEARLY a heuristic, not
        the same trained model used for Nifty 50.
        """
        import math
        d = data[['Open', 'High', 'Low', 'Close']].dropna().copy()
        if len(d) < 3:
            return 'UNKNOWN', 0.0, d
        d['PrevClose'] = d['Close'].shift(1)
        d['DayChangePct'] = (d['Close'] - d['PrevClose']) / d['PrevClose'] * 100
        d['GapPct'] = (d['Open'] - d['PrevClose']) / d['PrevClose'] * 100
        last_change = float(d['DayChangePct'].iloc[-1])
        vol = float(d['DayChangePct'].dropna().tail(10).std() or 1.0)
        # Momentum score: last day's move, normalised by recent volatility
        momentum_z = last_change / vol if vol else 0.0
        # Historical tendency: of the days that closed up, what fraction
        # gapped up the NEXT day (and vice versa) — a simple real base rate.
        gaps = d['GapPct'].dropna()
        changes = d['DayChangePct'].shift(1).dropna()
        aligned = pd.concat([changes, gaps], axis=1, join='inner')
        aligned.columns = ['PrevDayChange', 'NextGap']
        if len(aligned) >= 5:
            up_after_up = aligned[(aligned['PrevDayChange'] > 0) & (aligned['NextGap'] > 0)].shape[0]
            all_up = aligned[aligned['PrevDayChange'] > 0].shape[0]
            base_rate_up = (up_after_up / all_up) if all_up else 0.5
        else:
            base_rate_up = 0.5
        # Combine momentum + base rate into a single score in [0,1] (prob of gap UP)
        score = 1 / (1 + math.exp(-momentum_z)) * 0.6 + base_rate_up * 0.4
        score = min(max(score, 0.0), 1.0)
        trend = 'BULLISH' if score >= 0.5 else 'BEARISH'
        confidence = round(abs(score - 0.5) * 2 * 100, 3)  # distance from coin-flip, scaled to 0-100
        return trend, confidence, d.tail(5)
