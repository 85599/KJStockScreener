...
class tools:
    ...
    # Get Daily Bank Nifty Index (for the gap-prediction heuristic)
    def fetchLatestBankNiftyDaily(self, proxyServer=None):
        data = yf.download(
                auto_adjust=False,
                tickers="^NSEBANK",
                period='1mo',
    ...
    # Get Daily Sensex Index (for the gap-prediction heuristic)
    def fetchLatestSensexDaily(self, proxyServer=None):
        data = yf.download(
                auto_adjust=False,
                tickers="^BSESN",
                period='1mo',
    ...
    # Get Daily OHLC for any individual NSE stock (for the gap-prediction
    # heuristic when the user picks their own stock instead of an index).
    def fetchLatestStockDaily(self, stockCode, proxyServer=None):
        symbol = stockCode.strip().upper()
        ...
        if data is None or data.empty:
            raise StockDataEmptyException(f"No data found for '{stockCode}'. Check the NSE symbol and try again.")
        data = self.makeDataBackwardCompatible(data)
        return data
