"""
Screener Tools for KJScreener Agent Harness.
...
def screen_52_week_high(index: str = "Nifty 500", within_pct: float = 3.0) -> str:
    """Screen stocks trading at or near their 52-week (1-year) high — real
    price data, not a guess. Use this whenever asked about stocks making
    52-week highs / new highs, since no other screener tool covers this.

    Args:
        index: Market index to screen
        within_pct: How close to the 52-week high counts as "near" (e.g. 3.0
            means within 3% of the highest close in the last year)
    """
    try:
        import classes.ConfigManager as ConfigManager
        import classes.Fetcher as Fetcher
        ...
        stock_list = fetcher.fetchStockCodes(ticker_option)
        if not stock_list:
            return f"No stocks found for index '{index}'."
        ...
        for symbol in stock_list[:50]:  # Limit for agent responsiveness
            try:
                stock_data_tuple = fetcher.fetchStockData(
                    symbol, '1y', configManager.duration,
                    proxyServer=None,
                    screenResultsCounter=_results_counter,
                    screenCounter=_screen_counter,
                    totalSymbols=len(stock_list),
                ...
                ltp = float(stock_data['Close'].iloc[-1])
                if high_52w <= 0:
                    continue
                pct_from_high = (high_52w - ltp) / high_52w * 100
                if pct_from_high <= within_pct:
                    matches.append(
                        f"{symbol}: LTP={ltp:.2f}, 52wk High={high_52w:.2f}, "
                        f"{pct_from_high:.2f}% below high"
                    )
            except Exception as e:
                logger.debug(f"Skipping {symbol} in 52-week-high screen: {e}")
                continue

        if not matches:
            return f"No stocks in {index} are within {within_pct}% of their 52-week high right now."
        return f"Stocks in {index} within {within_pct}% of their 52-week high:\n" + "\n".join(matches)
    except Exception as e:
        logger.error(f"screen_52_week_high error: {e}")
        return f"Could not run the 52-week-high screen: {e}"


def get_index_quote(index_name: str = "NIFTY 50") -> str:
    """Get the REAL current price, day change %, and 20/50-day SMA for an NSE
    index (NIFTY 50, NIFTY BANK, NIFTY IT, INDIA VIX, SENSEX, etc.) via
    yfinance. ALWAYS call this tool when asked about an index level, trend, or
    support/resistance instead of guessing or estimating a number — the
    screener tools only cover individual stocks, not the index itself, so
    without this tool there is no real index data available.

    Args:
        index_name: Name of the index, e.g. "NIFTY 50", "NIFTY BANK", "SENSEX"
    """
    ...
    symbol_map = {
        ...
        "NIFTY 100": "^CNX100", "NIFTY 200": "^CNX200", "NIFTY 500": "^CNX500",
        ...
        "NIFTY MEDIA": "^CNXMEDIA", "NIFTY PSU BANK": "^CNXPSUBANK",
        "INDIA VIX": "^INDIAVIX", "VIX": "^INDIAVIX",
        "SENSEX": "^BSESN",
    }
    key = (index_name or "NIFTY 50").strip().upper()
    ...
    if symbol is None:
        return (
            f"Unknown index '{index_name}'. Known indices: "
            + ", ".join(symbol_map.keys())
            + ". Do not guess a number for an unrecognised index — say it isn't available."
        )

    try:
        hist = yf.download(symbol, period="4mo", interval="1d",
                            progress=False, auto_adjust=False)
        if hist is None or hist.empty:
            return (
                f"Live data for {key} is unavailable right now (empty response). "
                f"Do NOT invent a price — tell the user live index data could not be fetched."
            )
        ...
        ma20 = float(close.tail(20).mean()) if len(close) >= 5 else last
        ma50 = float(close.tail(50).mean()) if len(close) >= 5 else last
        as_of = close.index[-1].strftime('%Y-%m-%d')
        return (
            f"{key} ({symbol}) — LTP: {last:,.2f} | Day change: {chg:+.2f} ({chg_pct:+.2f}%) | "
            f"20-day SMA: {ma20:,.2f} | 50-day SMA: {ma50:,.2f} | As of: {as_of} (last close, yfinance)."
        )
    except Exception as e:
        logger.warning(f"get_index_quote failed for {key}: {e}")
        return (
            f"Could not fetch live data for {key} ({e}). "
            f"Do NOT guess a number — tell the user live index data is unavailable right now."
        )
...
TOOL_MAP = {
    ...
    'screen_52_week_high': screen_52_week_high,
}
